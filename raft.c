...
int numepochs = 300000,votes=0,numblocks=1000000,k=0;
...
// system("ps -F -C raft.out --cumulative");
system("ps -o %C -C raft.out --cumulative");
system("ps -o %x -C raft.out --cumulative");
    pthread_exit (NULL);
}
